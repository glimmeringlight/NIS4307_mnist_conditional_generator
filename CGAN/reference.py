    ...
    for epoch in range(num_epoch):
        ...
        for i, (x, _) in enumerate(train_loader):
            ...
            if (i + 1) % 100 == 0 or (i + 1) == len(train_loader):
                print(
                    'Epoch {:02d} / {} | Step {:04d} / {} | Loss_D {:.4f} | Loss_G {:.4f}'.format(
                        epoch + 1, num_epoch, i + 1, len(train_loader), total_loss_D / (i + 1), total_loss_G / (i + 1)))

        x = torch.randn(128, input_dim).to(device)
        img = G(x)
        save_image(img, './results/' + '%d_epoch.png' % epoch)

    torch.save(obj=G.state_dict(), f="../nets/Generator.pth")
    """
    加载参数：model.load_state_dict(torch.load("models/net.pth"))
    """
